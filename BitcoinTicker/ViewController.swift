...
class ViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        getCurrencyData(url: baseURL + currencyArray[0])
    ...
    func updateCurrencyData(json : JSON) {
        ...
        if let tempResult = json["last"].double {
            ...
            print(symbolArray[currentRow])
            bitcoinPriceLabel.text = symbolArray[currentRow] + String(tempResult)
            
        }else{
            bitcoinPriceLabel.text = "Service currently unavailable"
        }
    }
